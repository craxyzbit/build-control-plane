...

{
  ...
  printf 'fi\n'
  printf 'git -C "${SOURCE_DIR}" reset --hard\n'
  printf 'git -C "${SOURCE_DIR}" clean -fdx\n'
  printf 'git -C "${SOURCE_DIR}" fetch --tags origin\n'
  printf 'git -C "${SOURCE_DIR}" checkout -f "${GIT_REF}"\n'
  printf 'if [[ "${GIT_REF_KIND}" == "branch" ]]; then\n'
